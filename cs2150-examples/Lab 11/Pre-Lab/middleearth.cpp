...
/** \brief Constructor for middleEarth, Illuvatar
 *
 *sets up the random seed
 *counts number of cities in the array
 *Selects the num_cities names of the cities that we will be using
 *Computes random city positions
 *computes the 2-d distance array
 *create hash of indices so we don't have to do a linear search each time we want to find a city name to index mapping
 *
 *\param xsize size of Middle-Earth x axis
 *\param ysize size of Middle-Earth y axis
 *\param num_cities number of cities in Middle-Earth
 *\param seed used in construction of Middle Earth]
 *
 */
...
/**\brief destructor for Middle-Earth, Sauron
 *
 */
...
/**\brief prints cities of Middle-Earth, mouth of Sauron
 *
 */
...
/**\brief prints table of the distances
 *
 */
...
/**\brief prints distance between to passed cities
 *
 ...
 *will be length+1 entries in the returned vector).
 *
 *\param length number of cities to visit (must be smaller than num cities)
