/* binarysearchtree.cpp: method implementations for binary search tree Robert Alexander, RMA3MZ */
...
int BinarySearchTree::int_path_length(BinaryNode *t, int depth)
{
  ...
  int r=0;
  if(t->left!=NULL){
    l+=int_path_length(t->left,depth+1);
    d+=depth;}
  if(t->right!=NULL){
    r+=int_path_length(t->right,depth+1);
    d+=depth;}
  return (d+r+l);
}
